...
# 生产随机数据
class CRandData:
    cn_last_names = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许',
                     ...
                     '熊', '纪', '舒', '屈', '项', '祝', '董', '梁']
    cn_first_name = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地', '为',
                     ...
                     '乾', '坤']
    cn_province = ('北京市', '上海市', '天津市', '重庆市', '安徽省', '福建省', '甘肃省', '广东省', '广西省', '贵州省', '河北省', '河南省',
                   '黑龙江省', '湖北省', '湖南省', '吉林省', '江苏省', '江西省', '辽宁省', '内蒙古省', '宁夏省', '青海省', '山东省', '山西省',
                   '陕西省', '四川省', '西藏省', '新疆省', '云南省', '浙江省', '海南省', '香港省', '台湾省')

    # 中文地址
    def cn_addr(self):
        pro = random.choice(self.cn_province)
        num = random.randint(2, 4)
        city = self.cns(num) + '市'
        other = self.cns()
        return pro+city+other

    # 单词
    def en_word(self):
        wl = random.randint(3, 10)
        return self.en(wl)

    # 中文姓名
    def cn_name(self):
        x = random.choice(self.cn_last_names)
        num = random.randint(2, 3)
        m = random.choice(self.cn_first_name)
        return x+m
    ...
    def user_name(self):
        ...
        if num == 1:
            ...
            return self.en_sequ(1, 3)

    # 英文串
    def en_sequ(self, minl=2, maxl=8):
        num = random.randint(minl, maxl)
        ret = []
        for i in range(num):
            ret.append(self.en_word())
        return ' '.join(ret)

    # 英文地址
    def addr(self):
        return self.en_sequ(4, 10)
    ...
    # 手机号
    def phone(self, ln=11):
        return '1' + self.digit(ln-1)

    # url地址
    def url(self, pf='http'):
        num = random.randint(1, 4)
        dat = ['www'+self.en_word()+'com']
        for i in range(num):
            dat.append(self.en_word())
        return pf + '://' + '/'.join(dat)
    ...
    @staticmethod
    def cn():
        hi = random.randint(0xb0, 0xf7)
        lo = random.randint(0xa1, 0xf0)
        val = f'{hi:x}{lo:x}'
        cn = bytes.fromhex(val).decode('gb2312')
        return cn.encode().decode()

    @staticmethod
    def cns(num=1):
        return ''.join([CRandData.cn() for c in range(num)])

    @staticmethod
    def en(num=1):
        cs = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
        return ''.join([random.choice(cs) for c in range(num)])
